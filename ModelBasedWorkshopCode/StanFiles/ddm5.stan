...
model {
  ...
  if (parallel) {
    target += reduce_sum(partial_sum_fullddm, rt, 1,
      a, t0_m, 0, zr_m, 0, v_m, v_s, resp, cnd); // st0 set to zero
  } else {
    for (i in 1:N) {
      if (resp[i] == 1) {
        // upper threshold
        target += wiener_lpdf(rt[i] | a, t0_m, zr_m, v_m[cnd[i]], v_s,  0, 0); // st0 and sd of starting point set to zero
      } else {
        // lower threshold (mirror drift and starting point!)
        target += wiener_lpdf(rt[i] | a, t0_m, 1-zr_m, -v_m[cnd[i]], v_s,  0, 0); // st0 and sd of starting point set to zero
      }
    }
  }
}
